    ...
    if (node->funcdecllist != nullptr)
    {
        for (auto func : *node->funcdecllist)
        {
            ...
            if (visit_result != nullptr)
                element->InsertEndChild(visit_result);
        }
    }
...
void Tree2XML::visit(FuncDecl *node)
{
    ...
    if (node->args != nullptr)
    {
        ...
        {
            string key = "arg_temp_" + to_string(idx++);
            element->SetAttribute(key.c_str(), arg->name());
        }
    }
    if (node->stm != nullptr)
    {
...
void Tree2XML::visit(Call *node)
{
    ...
    element->SetAttribute("type", tree::typeToString(node->type).c_str());
    ...
    if (node->args != nullptr)
    {
        for (auto arg : *node->args)
        {
            if (arg != nullptr)
            {
                arg->accept(*this);
                if (visit_result != nullptr)
                    argsElement->InsertEndChild(visit_result);
            }
        }
    }
